...
class MemoryPool:
    def __init__(self, save_path="data/", memory_file_path=None, hnsw_file_path=None, memory_list_init_path=None,
        ...
        :param save_path: path to directory where memory files are saved.
        :param memory_file_path: path to file with JSON dict[int memory id, Memory]
        :param hnsw_file_path: path to Hierarchical neighborhood small world file generated by hnswlib.
        :param memory_list_init_path: path to a text file with memories to add to the bot's memories on separate lines.
        :param clear_duplicated_memories: if true, the memory file loaded will be cleaned of duplicated memories.
        """
        self.save_path = save_path
        if not memory_file_path:
            memory_file_path = MEMORY_FILENAME
        if not hnsw_file_path:
            hnsw_file_path = HNSW_FILENAME
        self.memory_file_path = save_path + memory_file_path
        self.hnsw_file_path = save_path + hnsw_file_path
    ...
    def clear_duplicate_memories(self):
        "Clean out duplicated memories from loaded memories and save it to disk (overwrite with cleaned memories)."
        delete_ids = []
        traversed_ids = []
